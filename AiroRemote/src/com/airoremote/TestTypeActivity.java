...
public class TestTypeActivity extends AppActivity {
	...
	private String sIMEI, sUserName;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		sUserName = pref.getUsername();
		//System.out.println("yaha he kya??? "+pref.getisTestAutoStart());
		initLayouts();
	...
	private void initLayouts() {
		...
		if(pref.getisTestAutoStart().equals(SERVER_START_MESSAGE)){
			//autoStartDownloadConfig(activity);
			/*System.out.println("yaha he kya??? SERVER_START_MESSAGE "+pref.getCurrentTestName());
			new Thread(new Runnable() {
			    //Thread to stop network calls on the UI thread
			    public void run() {
			        APIManager apimanager = new APIManager(activity);
					apimanager.processUpdateTestStatus(sIMEI, sUserName, pref.getCurrentTestName(), true);
			    }
			}).start();	*/	
			 rbDownloadConfigurationTest.setChecked(true);
			}
		if(pref.isTestRunning())
		{
			...
			new Thread(new Runnable() {
			    ...
			    public void run() {
			        APIManager apimanager = new APIManager(activity);
					apimanager.processUpdateTestStatus(sIMEI, sUserName, pref.getValue(Preferences.KEY_TEST_NAME,""), false);
			    }
			}).start();
			}
			}
	...
	public void autoStartDownloadConfig(Context context)
	{	context = activity;
	...
	public void doDownload(final String sUsername, final String sPassword,
				...
				try {
					...
					if (status == APIManager.Status.ERROR) {
						...
						if (sResponse.trim().equalsIgnoreCase(
							...
							//System.out.println("sConfigContent "+sConfigContent);
							objTestConfig = parser.parseXML(sConfigContent);
							// objTestConfig.getIsExternalTest();
							if(pref.getisTestAutoStart().equals(SERVER_START_MESSAGE)){
								//System.out.println("pref.getCurrentTestName() "+pref.getValue(Preferences.KEY_TEST_NAME,""));
								apiClient.processUpdateTestStatus(sIMEI, sUserName, pref.getValue(Preferences.KEY_TEST_NAME,""), true);
							}
						} else {
	...
	private Handler progressHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (sResponseCode.equals(ResponseCodes.SUCCESS)) {
				//Checks weather test starts from Server or by user	
				if(pref.getisTestAutoStart()==""){
