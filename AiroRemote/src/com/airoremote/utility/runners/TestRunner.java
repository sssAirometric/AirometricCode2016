...
import com.airoremote.PermissionsActivity;
...
public class TestRunner {
	...
	void startMTTest() {
		if (!pref.isTestCanceled()) {
			L.event("startMTTest()...");
			if (objMTConfig != null) {
				//we have to put conditions there to check 
				//PermissionsActivity pa = new PermissionsActivity();
				//pa.insertDummyContactWrapper();
				//Intent i = new Intent(activity,PermissionsActivity.class);
				//activity.startActivity(i);
				//System.out.println("ye bta kya arha he..."+status);
				MTTestRunner mtTestRunner = new MTTestRunner(activity,testconfig) {
					@Override
					public void doResultUploaded(String code, String desc) {
				...
				/*if (activity.checkSelfPermission(Manifest.permission.MODIFY_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {
					if (!activity.shouldShowRequestPermissionRationale(Manifest.permission.MODIFY_PHONE_STATE)) {

				       // No explanation needed, we can request the permission.
						activity.requestPermissions(new String[]{Manifest.permission.MODIFY_PHONE_STATE},
	...
	/*public void onRequestPermissionsResult(int requestCode,
	        String permissions[], int[] grantResults) {System.out.println("r u der??111");
	    switch (requestCode) {
