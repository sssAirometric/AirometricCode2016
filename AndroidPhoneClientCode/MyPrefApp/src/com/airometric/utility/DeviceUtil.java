...
public class DeviceUtil {
	...
	private static final String WIFI_INFO_START = "<wifiinfo>";
	private static final String WIFI_INFO_END = "</wifiinfo>";

	private static final String BSSID_START = "<bssid>";
	private static final String BSSID_END = "</bssid>";
	private static final String WIFI_RSSI_START = "<rssi>";
	private static final String WIFI_RSSI_END = "</rssi>";

	private static final String WIFI_LINK_SPEED_START = "<linkspeed>";
	private static final String WIFI_LINK_SPEED_END = "</linkspeed>";

	// Added for LTE Network

	private static final String LTE_TAC_START = "<tac>";
	private static final String LTE_TAC_END = "</tac>";

	private static final String LTE_PCI_START = "<pci>";
	private static final String LTE_PCI_END = "</pci>";
	...
	public String getDeviceInfo(String sTestType, String sTestName,
		...
		/*Changed the test name for thin client code but revert back to the normal code as like Airometric coz have some problems 
		 * when xml file processed by mule on 21/06/2016.
		 * DateFormat dateFormat = new SimpleDateFormat("MMMdd HH:mma");
		Date date = new Date();
		
		String[] date1 = dateFormat.format(date).split(" ");
		sTestName=sTestName+"I"+device_id+"D"+date1[0]+"T"+date1[1];
		*/
	...
	public String getListenerInfo() {
		...
		if (wifi_state != null && wifi_state.equals("CONNECTED"))
			strBuffer.append("wifi");
		else
		...
		/* That is comment in the AM app thats why we need to comment it here as well.
		 * commented on 27/06/16 by ankit
		...
		if (objWifiInfo.getBSSID() != null)
			strBuffer.append(objWifiInfo.getBSSID());
		else
			strBuffer.append("NA");
	...
	public static class TcpStats {
		...
		public TcpStats(int sendCount, int receiveCount) {
	...
	public static TcpStats getStats(int uid) {
		TcpStats resTcpStats = null;
		...
		File uidActualSentFile = new File(uidFileDir, "tcp_snd");
		BufferedReader br = null, br1 = null;

		try {
			br = new BufferedReader(new FileReader(uidActualFile));
			br1 = new BufferedReader(new FileReader(uidActualSentFile));
			String line, line1;

			if ((line = br.readLine()) != null) {
				L.log(String.valueOf(uid) + " Rcv - " + line);// this returns
																// the amount of
																// data received
																// for the
																// particular
			...
			if ((line1 = br1.readLine()) != null) {
				L.log(String.valueOf(uid) + " Snd - " + line);// this returns
																// the amount of
																// data received
																// for the
																// particular
			...
			resTcpStats = new TcpStats(Integer.valueOf(line1),
					Integer.valueOf(line));
		} catch (IOException e) {
			e.printStackTrace();
			resTcpStats = new TcpStats(0, 0);
		} finally {
			try {
				if (br != null) {
					br.close();
